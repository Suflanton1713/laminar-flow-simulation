...
\usepackage{lmodern}
% \usepackage{microtype}  % Comentado para acelerar compilación en Overleaf
\usepackage{geometry}
\geometry{margin=1in}
...
\begin{titlepage}
    ...
    \vspace{0.25cm}
    {\Large\itshape Discretización de Navier--Stokes 2D, Mallas, Método de Newton--Raphson, solución iterativa de sistemas lineales y funciones de interpolación (Spline cúbico natural) \par}
    
    \vfill
    
    {\large\textbf{Presentado por:} \par}
    \vspace{0.5cm}
    {\large Juan David Rincón - 2342032\par}
    {\large Maria Juliana Saavedra - 2344035 \par}
    {\large Libardo Alejandro Quintero - 2342181\par}
    
    \vfill
    
    {\large 27 de noviembre de 2025 \par}
...
\section*{Resumen}
Se presenta la implementación completa de una simulación numérica de flujo laminar incompresible en 2D alrededor de dos vigas sumergidas, basada en el problema propuesto por Landau \& Páez. Partiendo del sistema de Navier--Stokes estacionario simplificado ($\nu=1$, $\nabla P=0$), se desarrolla la discretización por diferencias finitas centradas en malla uniforme ($h=8$), derivando las \textbf{nueve ecuaciones especiales} para el tratamiento de fronteras (cuatro lados, cuatro esquinas y condición de no deslizamiento en vigas).

El sistema no lineal resultante ($364$ ecuaciones) se resuelve mediante el \textbf{método de Newton--Raphson}, con una estrategia de inicialización informada que refleja la física del flujo (velocidades decrecientes de entrada a salida). Para resolver el sistema lineal en cada iteración, se analizaron cinco métodos iterativos: \emph{Richardson}, \emph{Jacobi}, \emph{Gauss-Seidel}, \emph{Gradiente Descendente} y \emph{Gradiente Conjugado}. El análisis del radio espectral demostró que los primeros cuatro métodos no convergen para nuestra matriz Jacobiana (no simétrica, no diagonalmente dominante).

La solución adoptada consiste en transformar el sistema $J\cdot H = -F$ al sistema equivalente $J^T J \cdot H = -J^T F$, donde $J^T J$ es simétrica y definida positiva (SDP), permitiendo aplicar el \textbf{Gradiente Conjugado}. Aunque el número de condición aumenta ($\kappa(J)\approx24 \to \kappa(J^TJ)\approx577$), el método converge en $\sim100$ iteraciones por paso de Newton, alcanzando convergencia total en $\sim116$ iteraciones de Newton-Raphson.

Finalmente, se implementó \textbf{interpolación con splines cúbicos bidimensionales} (RectBivariateSpline de SciPy) para generar visualizaciones suavizadas del campo de velocidades, incluyendo superficies 3D y perfiles de corte. El código Python resultante genera automáticamente valores iniciales y produce mapas de calor, visualizaciones del Jacobiano y campos de velocidades refinados.
\newpage
...
\begin{center}
\url{https://github.com/Suflanton1713/laminar-flow-simulation}
\end{center}

\subsection{Contenido del Repositorio}

\begin{itemize}
    \item \texttt{malla.py}: Código principal de la simulación en Python, incluyendo las clases \texttt{Bloque}, \texttt{Malla} y \texttt{Vector}, así como todas las funciones de visualización e interpolación.
    \item \texttt{informe.tex}: Este documento en formato \LaTeX.
    \item \texttt{README.md}: Documentación completa del proyecto.
    \item Imágenes generadas: Visualizaciones del Jacobiano, valores iniciales, superficie 3D del spline, cortes de interpolación y campo de velocidades suavizado.
    \item \texttt{deprecated\_initial\_values/}: Carpeta con valores iniciales de pruebas anteriores.
\end{itemize}

\subsection{Sobre el Modelo y los Datos Iniciales}

El modelo implementado está basado en el problema propuesto en el libro de \textbf{Landau \& Páez} \cite{LandauPaez} (Capítulo 4.6: Hidrodinámica). La geometría del canal, la posición de los obstáculos y los parámetros físicos son valores fijos establecidos según esta referencia.

Los \textbf{valores iniciales} para el método de Newton-Raphson son \textbf{generados automáticamente} por el código mediante un algoritmo que simula el comportamiento físico esperado del fluido: velocidades que decrecen de izquierda a derecha (pérdida de momento) y de arriba hacia abajo (perfil de velocidad). No se requiere ninguna configuración manual ni archivos de datos externos.

\subsection{Documentación en el README}

El archivo \texttt{README.md} contiene documentación detallada que incluye:

\begin{itemize}
    \item \textbf{Documentación de clases}: Descripción completa de las clases \texttt{Bloque}, \texttt{Malla} y \texttt{Vector} con sus parámetros, atributos y métodos.
    \item \textbf{Documentación de funciones}: Explicación de todas las funciones auxiliares de análisis, visualización e interpolación.
    \item \textbf{Diagrama del proceso \texttt{main()}}: Flujo de ejecución paso a paso desde la definición de geometría hasta la generación de resultados suavizados.
    \item \textbf{Instrucciones de instalación}: Requisitos y comandos para ejecutar la simulación.
    \item \textbf{Descripción de resultados}: Tabla con todos los archivos de imagen generados.
\end{itemize}
