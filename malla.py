...
def main():
    ...
    # Cargar o crear matriz inicial
    if os.path.exists("matriz_valores_iniciales.txt"):
        print("Cargando matriz inicial desde archivo...")
        matriz_cargada = np.loadtxt("matriz_valores_iniciales.txt", delimiter='\t')
        mallaInicial = Malla(400, 40, h, 1, bloqueSuperior, bloqueInferior, 10, 5, matriz_inicial=matriz_cargada)
    else:
    ...
    print("\n=== Visualización de Malla Inicial ===")
    mallaInicial.visualizar_malla(
        titulo="Malla de valores iniciales - Simulación",
        guardar=True,
        nombre_archivo="malla_inicial.png",
    ...
    # Calcular jacobiana inicial
    xinit.cal_jacobiano(mallaInicial)
    xinit.visualizar_jacobiana(guardar=True)
